...
export function DetectorForm() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const isAuthentic = v.is_authentic;
      const confidence = isAuthentic ? 95 : 85;
  ...
  return (
    <div className="w-full max-w-3xl">
        ...
        <div className={`border-2 border-dashed rounded-lg p-8 text-center ${image ? 'border-green-400' : 'border-gray-300'}`}>
          ...
          {image && (
            <button
              ...
              className={`ml-4 inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors ${isAnalyzing ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
              {isAnalyzing ? (
                <>
      ...
      {result && (
        <div className={`p-6 rounded-lg shadow-md ${result.isAuthentic ? 'bg-green-50' : 'bg-red-50'}`}>
            ...
            <div>
              <p className="text-gray-700">
                ...
                <span className={`font-bold ${result.isAuthentic ? 'text-green-600' : 'text-red-600'}`}>
                  {result.isAuthentic ? 'likely authentic' : 'likely AI-generated'}
                </span>
              </p>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Watermark: {result.watermark_found ? '✔️ Found' : '❌ Not found'}
              </p>
              <p className="text-sm text-gray-500 mt-1">
              ...
              {result.watermark_content && (
                <p className="text-sm text-gray-400 mt-2">Watermark Content: <code>{result.watermark_content}</code></p>
              )}
            </div>
            <div className={`w-16 h-16 rounded-full flex items-center justify-center ${result.isAuthentic ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
